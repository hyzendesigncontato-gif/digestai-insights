-- ============================================
-- DigestAI - Database Functions & Triggers
-- Funções e gatilhos para automação
-- ============================================
...
-- Atualiza scores de todos os alimentos de um usuário
-- ============================================
CREATE OR REPLACE FUNCTION update_user_food_status_scores(p_user_id UUID)
RETURNS INTEGER AS $$
DECLARE
    v_food RECORD;
    v_score INTEGER;
...
BEGIN
    ...
    LOOP
        -- Calcula o score
        v_score := calculate_food_safety_score(p_user_id, v_food.id);
        ...
        VALUES (p_user_id, v_food.id, v_status, v_score)
...
-- Analisa possíveis intolerâncias de um usuário
-- ============================================
...
BEGIN
    ...
    LEFT JOIN food_logs fl ON fl.user_id = p_user_id 
...
BEGIN
    ...
    INTO v_intolerances
    FROM get_intolerance_analysis(p_user_id, p_period_days);
...
BEGIN
    SELECT jsonb_build_object(
        ...
        'safeFoods', (
            ...
            WHERE user_id = p_user_id AND status = 'safe'
        ),
        'avoidFoods', (
            SELECT COUNT(*) 
            FROM user_food_status 
            WHERE user_id = p_user_id AND status = 'avoid'
        ),
        'moderateFoods', (
            SELECT COUNT(*) 
            FROM user_food_status 
            WHERE user_id = p_user_id AND status = 'moderate'
        ),
        'latestReport', (
            SELECT jsonb_build_object(
                'id', id,
...
BEGIN
    ...
    IF p_priority = 'high' THEN
        ...
        VALUES (p_user_id, 'insight', p_title, p_content);
    END IF;
